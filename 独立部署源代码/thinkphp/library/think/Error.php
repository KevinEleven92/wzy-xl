...
class Error
{
    ...
    public static function appException($e)
    {
        ...
        if($e instanceof ErrorException){
            //错误处理
            self::sysErrExp($e->getSeverity(), $e->getMessage(), $e->getFile(), $e->getLine());
        }else{
            self::sysErrExp(-1, $e->getMessage(), $e->getFile(), $e->getLine(), $e->getTraceAsString());
        }
    ...
    public static function appError($errno, $errstr, $errfile = '', $errline = 0)
    {
        ...
        if (error_reporting() & $errno) {
            ...
            //不抛出异常终止，仅记录错误
            self::sysErrExp($exception->getSeverity(), $exception->getMessage(), $exception->getFile(), $exception->getLine());
        }
    ...
    public static function appShutdown()
    {
        ...
        if (!is_null($error = error_get_last()) && self::isFatal($error['type'])) {
            self::sysErrExp($error['type'], $error['message'], $error['file'], $error['line']);
    ...
    public static function sysErrExp($severity, $message, $file, $line, $trace=''){
        $severityDefs = [
            -1 => "E_EXCEPTION",
            E_ERROR => "E_ERROR",
            E_WARNING => "E_WARNING",
